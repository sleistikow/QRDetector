...
Bei der Lokalisierung geht es um die Verortung des QR-Codes. Dafür wird nun nicht mehr das Eingabebild selbst, sondern das Ergebnis der Vorverarbeitung verwendet.
Grund dafür ist, dass der \emph{Canny}-Operator auf das Bild angewendet wird, um die Konturen zu erhalten. Dieser funktioniert am besten bei hohem Kontrast.

\begin{figure}[h]
	\begin{center}
		\subfigure[Konturen eines Positionsmarkers]{
			\includegraphics[width=0.45\linewidth]{./images/area.png}
			\label{fig:konturen}
		}
		\subfigure[Flächenverhältnisse]{
			\includegraphics[width=0.45\linewidth]{./images/area.png}
			\label{fig:flaecheninhalte}
		}
	\end{center}
	\caption{TODO}
	\label{fig:positionsmarker}
\end{figure}

\subsubsection{Geometrische Normalisierung}

%Zunächst muss betrachtet werden, wie die Position und Ausrichtung eines QR-Codes definiert sind.
